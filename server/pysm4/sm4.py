...
def _round_f(byte4_array, rk):
    """
    ?ֺ???, F(X0, X1, X2, X3, rk) = X0 ?? T(X1 ?? X2 ?? X3 ?? rk)
    :param byte4_array: (X0, X1, X2, X3), ÿһ??4byte, 32bit
    :param rk: ????Կ, 4byte, 32bit
    """
...
def encrypt(clear_num, mk):
    """
    SM4?????㷨??32?ε?????????1?η????任R????.
    ????????Ϊ(X0, X1, X2, X3), ÿһ??4byte, ????????Ϊ(Y0, Y1, Y2, Y3), ÿһ??4byte
    ????ԿΪrki, i=0,1,...,32, 4byte, ????????????:
    1). 32?ε???????: Xi+4 = F(Xi, Xi+1, Xi+2, Xi+3, rki), i=0,1,...,32
    2). ?????任: (Y0, Y1, Y2, Y3) = (X35, X34, X33, X32)
...
def decrypt(cipher_num, mk):
    """
    SM4?????㷨, ???ܱ任?????ܱ任?ṹ??ͬ, ??ͬ?Ľ???????Կ??ʹ??˳??.
    ????ʱ????Կʹ??˳??Ϊ(rk31,rk30,...,rk0)
